...
- **邮箱**：对接**自建临时邮 API**（兼容 cloudflare_temp_email）；内置公共临时邮已移除
...
```json
{
  ...
  "cloudflare_api_key": "your-admin-token",
...
| `email_provider` | 固定走自建通道（`cloudflare` / 面板「自定义」） |
| `cloudflare_api_base` | **必填**：自建临时邮 API 根地址 |
| `cloudflare_api_key` | 管理密钥 / Admin Token（视服务而定） |
| `defaultDomains` | 邮箱域名（可空则服务端默认） |
...
> **内置公共临时邮已移除。**  
> 因大规模滥用，Tempmailer 等公共源已暂时拒收 xAI 验证码邮件（页面提示类似：  
> `Due to recent large-scale abuse, emails from xAI are temporarily not accepted.`）。  
> 请自行部署可收 xAI 邮件的临时邮 API，再在面板填写。

| 选项 | 说明 |
|------|------|
| 自定义 / 自建 | 兼容 [cloudflare_temp_email](https://github.com/dreamhunter2333/cloudflare_temp_email) 一类：API 根地址 / Key / 域名 / 路径 |

自建服务需支持：

1. **创建地址**（拿到邮箱 + token/jwt）  
2. **收信读验证码**（轮询邮件正文提取 xAI 验证码）

推荐：自己的域名 + Cloudflare Email Routing + Worker 临时邮。
...
### v1.1.1（2026-07-17）
- **移除内置 Tempmailer / 公共临时邮**：因滥用，服务商已拒收 xAI 验证码邮件
