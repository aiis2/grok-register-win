...
| 凭据 | 统一存储目录、容量统计、手动迁移、CPA OAuth 并发度、队列状态、补转与全部 SSO 刷新 |
...
在新版面板进入“凭据 → CPA 转换”，点击“刷新全部 SSO 换票”；经典界面提供同名回退按钮。该操作使用当前账号池里已经保存的 Web SSO 重新换取 OAuth/CPA，**不会生成新的 Web SSO**。处理上限支持 `1–10000`。OAuth 网络阶段默认使用 2 个 worker，可在新版页面设置为 `1–4`；凭据文件、索引和失败记录始终由单提交器串行落盘，同一 SSO 在兑换或待提交阶段都不会重复入队。
