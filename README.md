...
[![Version](https://img.shields.io/badge/version-v1.5.0-111827)](https://github.com/aiis2/grok-register-win/releases/tag/v1.5.0)
[![Python](https://img.shields.io/badge/Python-3.10%2B-3776AB?logo=python&logoColor=white)](https://www.python.org/downloads/)
[![Windows](https://img.shields.io/badge/Windows-10%20%7C%2011-0078D4?logo=windows)](#运行要求)
[![License](https://img.shields.io/badge/License-MIT-16A34A)](LICENSE)

1–10 并发注册 · 独立有头 Chromium · Cloudflare Temp Email · 端到端收件验证 · 可迁移凭据仓库 · SSO / CPA / Sub2 导出
...
- 支持 `1–10` 个固定注册槽；每槽独占 CLI 进程、有头浏览器、调试端口和临时 Profile；
- 槽内复用同一个健康 Chromium，账号间清理 Cookie、缓存和站点存储；
...
7. 选择 Chromium 或 Camoufox，填写注册数量和 `1–10` 的并发度后启动任务。
8. 在 Worker Process Matrix 中查看每个浏览器槽的范围、PID、当前轮次和结果。
...
| 引擎 | 模式 | 说明 |
| --- | --- | --- |
| Chromium | 有头 | 默认使用系统 Chrome/Edge；每个并发槽独占浏览器，槽内账号复用根进程 |
...
| 环境变量 | 对应配置 | 说明 |
| --- | --- | --- |
| `MAIL_WEB_URL` | `freemail_api_url` | Freemail 服务根地址 |
| `ADMIN_NAME` | `freemail_username` | 管理员用户名 |
| `ADMIN_PASSWORD` | `freemail_password` | 管理员密码 |

页面中的非空显式配置优先于环境变量；环境变量只在运行时合并，不会因为测试而写入 `config.json`。密码输入框留空会保留已有配置，接口只返回“已配置/环境变量可用”的布尔状态，不会回传密码。原生发件测试还要求 Freemail 登录响应中的 `can_send` 已开启，并且服务端配置了可用发件渠道。
...
| `MAIL_WEB_URL` | 未设置 | Freemail API URL；页面配置优先 |
