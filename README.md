...
[![Version](https://img.shields.io/badge/version-v1.14.0-111827)](https://github.com/aiis2/grok-register-win/releases/tag/v1.14.0)
[![Python](https://img.shields.io/badge/Python-3.10%2B-3776AB?logo=python&logoColor=white)](https://www.python.org/downloads/)
[![Windows](https://img.shields.io/badge/Windows-10%20%7C%2011-0078D4?logo=windows)](#运行要求)
[![License](https://img.shields.io/badge/License-MIT-16A34A)](LICENSE)

1–10 并发注册 · 隐藏有头 Chromium · Cloudflare Temp Email · 端到端收件验证 · 可迁移凭据仓库 · Access denied 禁用池 · 账号重新授权 · 单实例 OAuth 所有权 · CPA / Sub2 导出
...
- 以默认 2、可配置 `1–4` 的有界 OAuth worker 自动转换 CPA，并由单提交器安全生成 CPA JSON 与 Sub2API 导入包；
- 可在新版或经典界面用已有 Web SSO 重新生成 OAuth/CPA 授权，最多一次排队 `10000` 个稳定身份；成功原子替换，失败保留旧 CPA；
- OAuth consent 动态识别当前 `submitOAuth2Consent` Server Action，支持 RSC 响应和回调重定向；明确区分 consent action、callback 与 token exchange 的授权拒绝，并在连续拒绝时暂停剩余 CPA，避免持续放大无效请求；
- OAuth 明确返回 `Access denied` 时把对应身份持久化到禁用账号池；禁用账号不会再参与重新授权、CPA 转换及任何 SSO、CPA、Sub2 或聚合导出，只能在页面中手动恢复；
- CPA / Sub2 导出绑定明确的目标实例，同一身份的 refresh token 不允许被不同实例重复认领；
...
| 账号 | 服务端搜索、来源/CPA 状态过滤、排序、`25/50/100` 分页、批次导入与导出、禁用账号查看与手动恢复 |
| 邮箱 | 全部邮箱适配器、安全密钥保存、连接测试、发件能力检查和端到端收件验证 |
| 凭据 | 统一存储目录、容量统计、手动迁移、CPA OAuth 并发度、目标实例、补转与账号重新授权 |
...
| `oauth_ownership.json` | OAuth 目标实例与授权代次登记（仅保存 SHA-256 指纹，不保存邮箱或 Token） | `data/credentials/cpa/` |
| `accounts.json` | `Access denied` 禁用账号登记（含恢复所需内部身份信息，页面 API 仅返回公共字段） | `data/credentials/disabled/` |
...
- **迁移并切换**：将当前目录及历史位置中的 SSO、邮箱 JWT、CPA 与禁用账号登记一起迁移。

迁移事务按以下顺序执行：

1. 按规范化绝对路径排序并锁定新旧 OAuth 归属账本；任一账本正被其他程序实例使用时直接取消；
2. 将全部候选文件复制到目标目录的同目录临时文件；
3. 对来源和副本计算 SHA-256，全部一致后才继续；
4. 原子更新 `config.json` 中的 `credentials_dir`；
5. 在仍持有账本锁时删除已验证的来源文件；删除失败会保留文件并在 UI 返回 warning，随后释放全部锁。

不会覆盖普通凭据文件：同名同内容会跳过副本并清理来源；同名不同内容会保存为 `原名-migrated-时间戳.ext`。`oauth_ownership.json` 属于安全账本，不会改名旁路保存；迁移器会把多个非冲突账本合并到目标目录的唯一标准文件，若同一身份或 token 的实例归属冲突则停止迁移并保留原配置与来源。复制、校验或账本合并失败时不会切换配置，也不会删除来源。

从 v1.3 或更早版本升级时，直接在面板输入目标目录并点击“迁移并切换”。迁移器会同时识别：

- 项目根目录的 `accounts_*.txt`；
- 项目根目录的 `mail_credentials*.txt`；
- 历史 `data/cpa/` 中的 `xai-*.json`、`index.json`、`failed.jsonl` 和 `oauth_ownership.json`；
- 当前凭据目录 `disabled/accounts.json` 中的禁用账号登记；
- 当前已配置凭据目录中的对应文件。
...
在新版面板进入“凭据 → CPA 转换”，点击“重新生成账号授权”；经典界面提供相同操作。该操作使用当前账号池中保存的 Web SSO 重新执行 OAuth authorization-code exchange，生成新的 access token 与 refresh token，**不会生成新的 Web SSO，也不是拿旧 refresh token 做刷新**。处理上限支持 `1–10000`。系统优先按 SSO 中的稳定 subject 去重，缺失时回退到规范化邮箱或 SSO 指纹，避免同一身份重复排队。

升级前生成、尚未带 `_authorization_id` / `_authorization_generation` / `_authorized_at` 的旧 CPA 状态未知，可能已经在外部实例发生过 token rotation，因此不能直接认领或导出。升级后请先执行一次“重新生成账号授权”，待转换队列完成后再绑定目标实例并导出。

OAuth 网络阶段默认使用 2 个 worker，可在新版页面设置为 `1–4`；同一面板进程内的 worker 共享一份工作区租约并保持并行，凭据文件、索引、授权代次和失败记录仍由单提交器串行落盘。租约从 authorization-code exchange 开始一直持有到原子提交结束；另一面板进程、导入、迁移或导出领取若同时使用该凭据目录，会在换取新 token 前失败关闭。任务在加锁入队时绑定规范化目录和持久化 workspace epoch；导入、迁移或目录切换会推进 epoch，尚未开始交换的旧任务将被丢弃。正常 CPA 提交只推进授权缓存版本，不会误杀同批后续任务；其他进程下次提交前会重扫授权代次，避免缓存过期后重复生成同一代次。

- 已完成的身份可强制重新授权，已在处理中的相同 SSO 会安全跳过；
- 每次成功授权写入新的 `_authorization_id`、`_authorization_generation` 与 `_authorized_at`；
- 成功结果通过同目录临时文件原子替换现有 CPA；
- 授权或写入失败时不删除、不截断旧 CPA，并在状态与脱敏日志中展示错误；
- 只对 429、短暂 5xx 和网络超时执行最多两次退避重试；SSO 失效、OAuth 协议错误和 Cloudflare 403 不盲目重试；
- 注册任务、凭据导入或凭据迁移正在切换时，重新授权请求返回明确的忙碌提示；
- API 和页面只返回总数、入队数、去重数与公共队列状态，不返回 SSO、密码或 OAuth Token。

#### Access denied 禁用账号池

OAuth consent 明确返回 `Access denied` / `access_denied` 时，系统会把该稳定身份写入凭据目录下的 `disabled/accounts.json`，并把已有 CPA 文件标记为 `disabled`。网络超时、429、5xx、解析错误或其他系统性故障不会误触发禁用。

禁用状态是所有读取路径的共同过滤条件：账号列表、重新授权候选、自动 CPA 队列、单个账号文件下载、合并 SSO、CPA ZIP、Sub2 ZIP/JSON、grok2api、全部产物 ZIP、失败记录导出和其他聚合格式都会排除该身份。即使旧 CPA 文件仍存在，文件内的 `disabled` 标记也会形成第二道保护。

新版面板“账号”页提供独立的禁用账号池、分页浏览和“恢复并重新授权”操作。页面只显示邮箱、原因、禁用时间和来源等公共信息，不返回密码、Web SSO 或 OAuth Token。恢复操作必须由用户确认；系统先从禁用登记移除身份，再强制进入重新授权队列，入队失败则回滚禁用状态。旧 CPA 在新的授权成功原子提交前仍保持禁用，因此恢复不会让旧 refresh token 重新进入导出。

#### 避免 refresh token 被多个实例同时使用

在“凭据 → CPA 转换”填写并保存 `OAuth 目标实例`，例如 `sub2api-primary`。CPA ZIP、Sub2 ZIP 和 Sub2 JSON 都必须先通过只读预检，再由 JSON POST 原子认领；认领成功后只返回一个 60 秒内有效、一次性且绑定具体产物的下载票据：

1. 第一次导出会把当前授权绑定到该目标实例；
2. 同一目标实例可以再次下载同一授权；
3. 同一 refresh token 若已属于其他身份或实例，导出会失败并要求先重新生成账号授权；
4. 新授权要迁移到另一目标实例时，必须先停用旧实例里的对应账号，再在确认框中明确确认；
5. `oauth_ownership.json` 只保存 identity/token 的 SHA-256 指纹、目标实例、授权 ID 与代次；它随凭据目录安全合并迁移，但批量导入账号时不会被归档、删除或重置；
6. 直接访问下载 GET、复用票据或把 Sub2 票据用于 CPA 下载都不会改变所有权，并会被拒绝。

目标实例标识不是密钥，但应保持稳定且能唯一代表一个实际消费端，例如一套 Sub2API 或 CLIProxyAPI 部署。请把当前凭据目录作为这些身份的唯一权威仓库，不要让多套 Grok Register 同时为同一身份生成或分发授权，也不要把同一导出包复制到多个实例。跨进程保护只对共享同一凭据目录且运行本版本的程序有效；应用可以阻止通过本面板重复导出，但无法撤回已经手工复制到面板之外的旧文件。因此升级后应停用旧面板进程，切换目标实例时也必须先停用旧实例账号。
...
| `cpa_oauth_concurrency` | `2` | CPA OAuth 网络 worker 数量，范围 `1–4`；修改后重启面板生效 |
| `credentials_dir` | `data/credentials` | SSO、邮箱 JWT 与 CPA 的统一根目录 |
| `oauth_target_instance` | 空 | CPA / Sub2 OAuth 凭据的唯一消费实例标识，1–64 位字母、数字、点、下划线或连字符 |
...
| `CPA_CONCURRENCY` | 未设置 | 覆盖页面保存的 CPA OAuth 并发度，范围 `1–4` |
