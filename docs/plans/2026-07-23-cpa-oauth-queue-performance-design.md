...
2026-07-23 完成实现后，隔离性能探针使用 24 个任务、固定 80ms 合成 OAuth 延迟和真实
文件/索引提交：

| OAuth worker | 总耗时 | 吞吐 | 相对单 worker |
| ---: | ---: | ---: | ---: |
| 1 | 2.418 秒 | 9.92/秒 | 1.00× |
| 2 | 1.219 秒 | 19.69/秒 | 1.98× |
| 4 | 0.647 秒 | 37.07/秒 | 3.74× |

三组均生成 24/24 个 CPA 文件和 24/24 条有效索引，成功 24、失败 0；结束后
`pending`、`active_workers`、`commit_pending`、`commit_active`、inflight 和线程存活
数均为 0。

真实 OAuth 对照使用两组各 4 个已存在且此前转换成功的 Web SSO，结果只记录匿名化
计数和耗时：

| OAuth worker | 成功/失败 | 总耗时 | 吞吐 | OAuth 中位/P95 |
| ---: | ---: | ---: | ---: | ---: |
| 1 | 4/0 | 16.378 秒 | 14.65/分钟 | 2.768 / 2.983 秒 |
| 2 | 4/0 | 7.180 秒 | 33.43/分钟 | 2.524 / 2.654 秒 |

并发 2 的端到端总耗时提升为 2.28×，两组索引均验证 4/4，未出现 429、403、超时或
协议错误，最终队列、inflight 和 worker/提交线程全部归零。因此保留默认值 2，并允许
用户在 `1–4` 之间配置。

新版页面通过真实 Playwright 验证：

- 凭据页显示 `OAuth 活跃 / 并发`、`等待提交` 和 `1–4` 并发输入；
- 保存 3 后正确提示重启生效，再保存回 2，运行值与保存值恢复一致；
- 深色主题切换正常；
- 浏览器控制台 0 error、0 warning；
- 面板最终只有一个 `127.0.0.1:8787` listener，运行并发 2，所有 CPA 状态计数为 0。
